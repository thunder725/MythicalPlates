...
public class AllmightySinnoh : SummoningModule {
    ...
    string[] plateNames = new string[18]{
        "Blank", "Fist", "Sky", "Toxic", "Earth", "Stone", "Insect", "Spooky", "Flame",
        "Splash", "Iron", "Meadow", "Zap", "Mind", "Icicle", "Draco", "Dread", "Pixie"
    };
    ...
    [SerializeField] AudioClip SolvablePlateSolvedSound, AllmightySinnohSolveSound;
    ...
    {
        AllmightySinnohModuleLog(moduleId, "!! STRIKE !!");
    ...
    void SpawnEighteenVisualPlates()
    {
        ...
        foreach (KMSelectable _plateSelectable in eighteenVisualPlateSelectables)
        {
            ...
            _plateSelectable.OnInteract += delegate () { PressedMarkedPlate(Array.IndexOf(eighteenVisualPlateSelectables, _plateSelectable)); return false; };
        }
    ...
    void RotatePlatesOnUpdate()
    {
        ...
        visualPlatesRotationTime = (visualPlatesRotationTime + (Time.deltaTime * platesRotationSpeed)) % 1;
    ...
    IEnumerator IncreaseVisualRotationSpeedForFeedback()
    {
        ...
        while (timer < 2)
        {
            ...
            visualPlatesRotationTime += Easing.InQuad(timer, -0.01f, 0, 2);
    ...
    void ApplyMarkSteps()
    {
        ...
        if (bombReference != null)
        {
            ...
            TimerModule _timerScript = bombReference.GetComponentInChildren<TimerModule>();
    ...
    {
        ReceiveTwitchHelpMessage("Press the three Marked Plates using “!{0} Submit Meadow Iron Pixie”. Show the 18 names for 1 second using “!{0} shownames”. Wiggle the bomb using “!{0} wiggle”. Press the SINNOH casing button using “!{0} sinnoh”.");
    }
    ...
    protected override IEnumerator ProcessTwitchCommand(string command)
    {
        ...
        // Wiggling the bomb to check the Marked by Time
        if (commandParts.Length == 1 && commandParts[0] == "wiggle")
        {
            ...
            float rotationFrequency = 25f;
            float rotationAmplitude = 5f;
        ...
        if (markToPress > 2)
        {
            AllmightySinnohModuleLog(moduleId, "Sending command");
            yield return currentSummonedPlateScript.ProcessTwitchCommand(command);
            yield break;
        }
        ...
        // Verify existance of the 3 submittes plates
    ...
    protected override IEnumerator TwitchHandleForcedSolve()
    {
        ...
        ModuleLog(moduleId, "Received instructions to Twitch Force Solve!");
        ...
        // Cannot just do for loop with 3 because we might receive an AutoSolve
        // After having solved one plate already!
        while (isModuleSolved == false)
        {
            AllmightySinnohModuleLog(moduleId, "Auto-Solving next Plate");

            // Allow the plate to still visually start appearing
            yield return new WaitForSeconds(0.5f);

            // Ask the Plate to forcesolve itself
            yield return currentSummonedPlateScript.TwitchHandleForcedSolve();
        }
